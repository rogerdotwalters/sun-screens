...
(function initCalculator() {
  ...
  // Assumption inputs
  const efficiencyInput = $('#screen-efficiency');
  const acFracInput     = $('#ac-fraction');
  const windowFacInput  = $('#window-factor');
  ...
  function calcSavings() {
    ...
    const efficiency  = Math.min(Math.max(parseFloat(efficiencyInput.value) / 100, 0.1), 0.95);
    const acFrac      = Math.min(Math.max(parseFloat(acFracInput.value) / 100, 0.1), 0.95);
    const windowFac   = Math.min(Math.max(parseFloat(windowFacInput.value) / 100, 0.05), 0.80);
    ...
    const baseMonthly = bill * acFrac * windowFac * efficiency * windowMultiplier;
  ...
  function updateLabels() {
    ...
    sizeDisplay.textContent    = size.toLocaleString('en-US') + ' sq ft';
    windowsDisplay.textContent = windows + ' window' + (windows === 1 ? '' : 's');

    [billSlider, sizeSlider, windowsSlider].forEach(updateTrack);
  }
  ...
  // Attach listeners
  [billSlider, sizeSlider, windowsSlider].forEach(s => {
    s.addEventListener('input', update);
  });
  [efficiencyInput, acFracInput, windowFacInput].forEach(i => {
...
(function initForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      data.append('homeSize',                $('#home-size')?.value             || '');
